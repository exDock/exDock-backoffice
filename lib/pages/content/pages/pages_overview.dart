...
class _PagesOverviewState extends State<PagesOverview> {
  ...
  Future<List<OverviewPageRow>> Function(
    ...
    Future<List<OverviewPageRow>> getRows(
      ...
      final List<dynamic> items = data["page_overview"]["attributes"][0]
          ["current_attribute_value"] as List<dynamic>;

      final List<OverviewPageRow> rows = items.map((item) {
        return OverviewPageRow(
          id: item["_id"],
          name: item["_id"],
          ...
          onSelect: () {
            router
                .push("/content/pages/${item["_id"].toString().substring(1)}");
          },
        );
      }).toList();
